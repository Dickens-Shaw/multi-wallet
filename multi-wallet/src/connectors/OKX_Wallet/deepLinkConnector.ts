...
const generateDeepLinkProvider = (info: WalletInfo) => {
	return {
		request: async ({ method, params }: any) => {
			return new Promise(() => {
				console.log('OKX------request', method, params);
				if (method === 'openDApp') {
					const url = `${info.deepLink}?dappUrl=${encodeURIComponent(window?.location.href)}`;
					console.log('OKX-----deepLink', url);
...
export default class OKXWalletDeepLinkConnector extends BaseConnector {
