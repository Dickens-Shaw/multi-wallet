...
const generateDeepLinkProvider = (info: WalletInfo) => {
	return {
		request: async ({ method, params }: any) => {
			return new Promise(() => {
				console.log('Trust------request', method, params);
				if (method === 'openDApp') {
					const url = `${info.deepLink}?url=${window.location.href}`; // &coin_id=60
					console.log('Trust------deepLink', url);
...
export default class BitgetWalletDeepLinkConnector extends BaseConnector {
