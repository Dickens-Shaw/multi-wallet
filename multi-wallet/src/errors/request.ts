...
export class HttpRequestError extends BaseError {
	...
	constructor({
		...
		super('HTTP request failed.', {
			cause,
			details,
			metaMessages: [status && `Status: ${status}`, body && `Request body: ${stringify(body)}`].filter(
				Boolean
			) as string[],
			name: 'HttpRequestError'
		});
...
export class WebSocketRequestError extends BaseError {
	constructor({
		...
		super('WebSocket request failed.', {
			cause,
			details,
			metaMessages: [`URL: ${getUrl(url)}`, body && `Request body: ${stringify(body)}`].filter(Boolean) as string[],
			name: 'WebSocketRequestError'
		});
	}
}
...
export class TimeoutError extends BaseError {
	constructor({ body, url }: { body: { [x: string]: unknown } | { [y: string]: unknown }[]; url: string }) {
